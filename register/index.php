...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty(trim($_POST["username"]))){
        ...
        $username_err = "Username cant have more than 50 characters.";
    }else{
    ...
    if(empty($username_err) && empty($password_err) && empty($confirm_password_err)){
        ...
        if($stmt = mysqli_prepare($link, $sql)){
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                mysqli_stmt_close($stmt);
                $sql = "SELECT id FROM users WHERE username='WWBuddy'";
                $result = mysqli_query($link, $sql);
                $row = mysqli_fetch_assoc($result);
                $wwbuddy = $row["id"];
                $message = "Hello, welcome to World Wide Buddy, here you can make friends all around the world. Just look for someone\'s profile and there you will be able to send them a message, enjoy your stay! :D";
                $sql = "INSERT INTO messages (sender, receiver, content) VALUES ('$wwbuddy', '$id', '$message')";
